import FetchFilters from '../widgets/fetchFilters';
...
class ChangeDetect {
	...
	postType(obj) {
		...
		const wrapper  = obj.closest('.pp-shortcode-form');
	...
	aspectRatio(obj) {
		if (obj.val()) {
			obj.closest('.pp-shortcode-form').find('.pp_crop_method').show();
		} else {
			obj.closest('.pp-shortcode-form').find('.pp_crop_method').hide();
		}
	}
	...
	toggleExcerptOptions(obj) {
		if (obj.val()) {
			obj.closest('.pp-shortcode-form').find('.pp_excerpt_length, .pp_excerpt_unit').hide();
		} else {
			obj.closest('.pp-shortcode-form').find('.pp_excerpt_length, .pp_excerpt_unit').show();
		}
	}
	...
	startWhen(obj) {
		...
		if (value && 'custom' === value) {
			obj.closest('.pp-shortcode-form').find('.pp_start_time').show();
		} else {
			obj.closest('.pp-shortcode-form').find('.pp_start_time').hide();
		}
	}
	...
	postFetch(obj) {
		// Fetch episodes list for episode filter option.
		this.fetch(obj.closest('.pp-shortcode-form'), 'post');
	}
	...
	feedFetch(obj, isReset = true) {
		// Fetch episodes list for episode filter option.
		this.fetch(obj.closest('.pp-shortcode-form'), 'feed', isReset);
	}
